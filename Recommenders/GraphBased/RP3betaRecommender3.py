...
class RP3betaRecommender3(BaseItemSimilarityMatrixRecommender):
    """RP3betaRecommender3 recommender"""

    RECOMMENDER_NAME = "RP3betaRecommender3"

    def __init__(self, URM_train, ICM=None, verbose=True):
        super(RP3betaRecommender3, self).__init__(URM_train, verbose=verbose)
        self.ICM = ICM
    ...
    def fit(
        ...
        # If ICM is provided, normalize it
        if self.ICM is not None:
            self.ICM = normalize(self.ICM, norm="l1", axis=0)
            Piu = self.ICM.T @ Pui
        else:
            # Fallback to original bipartite URM
            Piu = normalize(self.URM_train.T, norm="l1", axis=1)
        ...
        # Matrix multiplication for tripartite graph
        block_dim = 200
        d_t = Piu
        ...
        for current_block_start_row in range(0, Pui.shape[1], block_dim):
            ...
            if (
                time.time() - start_time_printBatch > 300
                or current_block_start_row + block_dim == Pui.shape[1]
            ):
