...
class SVDFeature(BaseRecommender):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        item_scores = (
            -np.ones((len(user_id_array), self.URM_train.shape[1]), dtype=np.float32)
            * np.inf
        )
    ...
    def _get_feature_format(self, userid, itemid):
        ...
        if userfeatures is not None:
            for j in range(len(userfeatures.indices)):
                output += "{:d}:{:.6f}\t".format(
                    userfeatures.indices[j] + self.n_users, userfeatures.data[j]
                )
        ...
        if itemfeatures is not None:
            for j in range(len(itemfeatures.indices)):
                output += "{:d}:{:.6f}\t".format(
                    itemfeatures.indices[j] + self.n_items, itemfeatures.data[j]
                )
