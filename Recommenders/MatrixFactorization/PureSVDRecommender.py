...
class PureSVDRecommender(BaseSVDRecommender):
    ...
    def fit(self, num_factors=100, random_seed=None):
        ...
        self._print(
            "Computing SVD decomposition... done in {:.2f} {}".format(
    ...
    while start_item < n_items:
        ...
        for col_index_in_block in range(this_block_weight.shape[0]):
            ...
            relevant_items_partition = np.argpartition(
                -this_column_weights, topK - 1, axis=0
            )[0:topK]
            ...
            similarity_builder.add_data_lists(
                ...
                col_list_to_add=np.ones(len(relevant_items_partition), dtype=np.int32)
                * item_original_index,
...
class ScaledPureSVDRecommender(PureSVDRecommender):
    ...
    def fit(
        self, num_factors=100, random_seed=None, scaling_items=1.0, scaling_users=1.0
    ):
        ...
        self.URM_train = user_scaling_matrix.dot(self.URM_train).dot(
            item_scaling_matrix
        )
