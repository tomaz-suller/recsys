...
class DataIO(object):
    ...
    def __init__(self, folder_path):
        ...
        if "\\..\\" in self.folder_path or "/../" in self.folder_path:
            raise ValueError(
                "To ensure portability path cannot contain relative paths such as '\\..\\' or '/../'"
            )
        ...
        if self._is_windows:
            ...
            if not os.path.isabs(self.folder_path):
                self.folder_path = (
                    self.folder_path
                    if self.folder_path[0] != "."
                    else self.folder_path[1:]
                )
                self.folder_path = (
                    self.folder_path
                    if self.folder_path[0] != "\\"
    ...
    def _check_dict_key_type(self, dict_to_save):
        ...
        assert (
            len(dict_to_save_key_str) == len(dict_to_save)
        ...
        try:
            ...
            for attrib_name, attrib_data in data_dict_to_save.items():
                ...
                if isinstance(attrib_data, DataFrame):
                    ...
                    attrib_data.to_csv(
                        current_temp_folder + "." + attrib_name + ".csv", index=True
                    )
            ...
            for attrib_name, attrib_data in attribute_to_save_as_json.items():
                ...
                with open(current_file_path + ".json", "w") as outfile:
                    ...
                    json.dump(
                        attrib_data, outfile, default=json_not_serializable_handler
                    )

            with zipfile.ZipFile(
                self.folder_path + file_name + ".temp",
                "w",
                compression=zipfile.ZIP_DEFLATED,
            ) as myzip:
                for file_to_compress in os.listdir(current_temp_folder):
                    myzip.write(
                        current_temp_folder + file_to_compress, arcname=file_to_compress
                    )
            ...
            os.replace(
                self.folder_path + file_name + ".temp", self.folder_path + file_name
            )
...
class MyTestCase(unittest.TestCase):
    def test_save_and_load(self):
        ...
        (
            self.assertTrue(
                (
                    original_data_dict["dataframe"].dtypes
                    == loaded_data_dict["dataframe"].dtypes
                ).all()
            ),
        ...
        (
            self.assertTrue(
                type(original_data_dict["dataframe"].loc[0, "I am a mess"])
                == type(loaded_data_dict["dataframe"].loc[0, "I am a mess"])
            ),
            "Datatypes are different",
        )
        (
            self.assertTrue(
                type(original_data_dict["dataframe"].loc[1, "I am a mess"])
                == type(loaded_data_dict["dataframe"].loc[1, "I am a mess"])
            ),
            "Datatypes are different",
        )
        (
            self.assertTrue(
                type(original_data_dict["dataframe"].loc[2, "I am a mess"])
                == type(loaded_data_dict["dataframe"].loc[2, "I am a mess"])
            ),
            "Datatypes are different",
        )
        (
            self.assertTrue(
                type(original_data_dict["dataframe"].loc[3, "I am a mess"])
                == type(loaded_data_dict["dataframe"].loc[3, "I am a mess"])
            ),
            "Datatypes are different",
        )
        ...
        self.assertTrue(
            np.array_equal(
                original_data_dict["sps_random"].toarray(),
                loaded_data_dict["sps_random"].toarray(),
            )
        )
        self.assertTrue(
            original_data_dict["result_folder_path"]
            == loaded_data_dict["result_folder_path"]
        )
        self.assertTrue(
            original_data_dict["cutoff_list_validation"]
            == loaded_data_dict["cutoff_list_validation"]
        )

        self.assertTrue(
            original_data_dict["dataframe"].equals(loaded_data_dict["dataframe"])
        )
        self.assertTrue(
            original_data_dict["multiindex_df_row"].equals(
                loaded_data_dict["multiindex_df_row"]
            )
        )
        self.assertTrue(
            original_data_dict["multiindex_df_col"].equals(
        ...
        self.assertEqual(
            original_data_dict["nested_dict"].keys(),
            loaded_data_dict["nested_dict"].keys(),
        )
        self.assertTrue(
            original_data_dict["nested_dict"]["A"]
            == loaded_data_dict["nested_dict"]["A"]
        )
        self.assertTrue(
            np.array_equal(
                original_data_dict["nested_dict"]["B"].toarray(),
