...
class MyTestCase(unittest.TestCase):
    def test_euclidean_similarity_integer(self):
        from Recommenders.Similarity.Compute_Similarity_Euclidean import (
        ...
        for vector1 in range(n_items):
            for vector2 in range(n_items):
                ...
                if vector1 == vector2:
                    assert (
                        W_local[vector1, vector2] == 0.0
                    ), "W_local[{},{}] not matching control".format(vector1, vector2)
    ...
    def test_euclidean_similarity_float(self):
        ...
        data_matrix = np.array(
            [[0.12, 0.0, 0.87, 1.0], [0.28, 0.8, 1.0, 0.69], [0.0, 0.37, 1.0, 0.01]]
        )
