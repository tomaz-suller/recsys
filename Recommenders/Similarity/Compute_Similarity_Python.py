...
class Incremental_Similarity_Builder:
    ...
    def __init__(
        self,
        matrix_size,
        initial_data_block=10000000,
        additional_data_block=10000000,
        dtype=np.float32,
    ):
        ...
        for data_point_index in range(len(row_list_to_add)):
            if self._next_cell_pointer == len(self._row_array):
                ...
                self._data_array = np.concatenate(
                    (
                        self._data_array,
                        np.zeros(self._additional_data_block, dtype=self._dtype_data),
                    )
                )
...
class Compute_Similarity_Python:
    def __init__(
        ...
        assert (
            topK >= 0 and topK <= self.n_columns
        ), "TopK must be between 0 and the number of columns"
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.normalize:
                    if self.asymmetric_cosine:
                        denominator = (
                            sum_of_squared_to_alpha[columnIndex]
                            * sum_of_squared_to_1_minus_alpha
                            + self.shrink
                            + 1e-6
                        )
                    else:
                        denominator = (
                            sum_of_squared[columnIndex] * sum_of_squared
                            + self.shrink
                            + 1e-6
                        )
                ...
                # Apply the specific denominator for Tanimoto
                    ...
                    denominator = (
                        this_column_weights
                        + (sum_of_squared[columnIndex] - this_column_weights)
                        * self.tversky_alpha
                ...
                if self.use_dense:
                    ...
                    relevant_items_partition = np.argpartition(
                        -this_column_weights, self.topK - 1, axis=0
                    )[0 : self.topK]
                    ...
                    if np.any(this_column_weights == 0.0):
                        ...
                        relevant_items_partition = relevant_items_partition[
                            non_zero_mask
                        ]
                        this_column_weights = this_column_weights[non_zero_mask]

                    similarity_builder.add_data_lists(
                        row_list_to_add=relevant_items_partition,
                        col_list_to_add=np.ones(
                            len(relevant_items_partition), dtype=np.int32
                        )
                        * columnIndex,
            ...
            if (
                ...
                print(
                    "Similarity column {} ({:4.1f}%), {:.2f} column/sec. Elapsed time {:.2f} {}".format(
                        processed_items,
                        processed_items / (end_col_local - start_col_local) * 100,
                        column_per_sec,
                        new_time_value,
                        new_time_unit,
                    )
