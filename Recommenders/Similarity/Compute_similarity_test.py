...
class MyTestCase(unittest.TestCase):
    def test_cosine_similarity_dense(self):
        ...
        W_dense_mul[
            np.arange(W_dense_mul.shape[0]), np.arange(W_dense_mul.shape[0])
        ] = 0.0

        assert np.all(
            W_dense_Cython == W_dense_mul
        ), "W_dense_Cython not matching control"
        assert np.all(
            W_dense_Python == W_dense_mul
        ), "W_dense_Python not matching control"
        assert np.all(
            W_dense_Parallel == W_dense_mul
        ), "W_dense_Parallel not matching control"

    def test_cosine_similarity_dense_row_weighted(self):
        from Base.Cython.cosine_similarity import (
            Cosine_Similarity as Cosine_Similarity_Cython,
        )
        from Base.cosine_similarity_parallel import (
        ...
        W_dense_mul = (
            data_matrix.dot(sps.diags(row_weights)).dot(data_matrix.T).toarray()
        )
        W_dense_mul[
            np.arange(W_dense_mul.shape[0]), np.arange(W_dense_mul.shape[0])
        ] = 0.0
        ...
        assert np.allclose(
            W_dense_Cython, W_dense_mul, atol=1e-4
        ), "W_dense_Cython not matching control"
        assert np.allclose(
            W_dense_Python, W_dense_mul, atol=1e-4
        ), "W_dense_Python not matching control"
        assert np.allclose(
            W_dense_Parallel, W_dense_mul, atol=1e-4
        ), "W_dense_Parallel not matching control"
    ...
    def test_cosine_similarity_dense_external_cfr(self):
        ...
        from sklearn.metrics.pairwise import (
            cosine_similarity as Cosine_Similarity_Sklearn,
        )
        ...
        W_dense_sklearn[
            np.arange(W_dense_sklearn.shape[0]), np.arange(W_dense_sklearn.shape[0])
        ] = 0.0

        assert np.allclose(
            W_dense_Cython, W_dense_sklearn, atol=1e-4
        ), "W_dense_Cython Cosine not matching Sklearn control"
        assert np.allclose(
            W_dense_Python, W_dense_sklearn, atol=1e-4
        ), "W_dense_Python Cosine not matching Sklearn control"
        assert np.allclose(
            W_dense_Parallel, W_dense_sklearn, atol=1e-4
        ), "W_dense_Parallel Cosine not matching Sklearn control"

        data_matrix = np.array([[1, 2, 0, 1], [0, 1, 4, 1], [1, 3, 1, 0]])
        data_matrix = sps.csr_matrix(data_matrix)

        cosine_similarity = Cosine_Similarity_Cython(
            data_matrix, topK=TopK, normalize=True, shrink=shrink, mode="jaccard"
        )
        W_dense_Cython = cosine_similarity.compute_similarity()
        ...
        assert np.allclose(
            W_dense_Cython, W_dense_Scipy, atol=1e-4
        ), "W_dense_Cython Jaccard not matching Scipy control"
        assert np.allclose(
            W_dense_Python, W_dense_Scipy, atol=1e-4
        ), "W_dense_Python Jaccard not matching Scipy control"
        assert np.allclose(
            W_dense_Parallel, W_dense_Scipy, atol=1e-4
        ), "W_dense_Parallel Jaccard not matching Scipy control"

    def test_cosine_similarity_dense_normalize(self):
        from Base.Cython.cosine_similarity import (
            Cosine_Similarity as Cosine_Similarity_Cython,
        )
        from Base.cosine_similarity import (
            Compute_Similarity as Cosine_Similarity_Python,
        )
        from Base.cosine_similarity_parallel import (
            Cosine_Similarity_Parallel as Cosine_Similarity_Parallel,
        )
        ...
        W_dense_denominator = (
            np.multiply(W_dense_denominator, W_dense_denominator.T) + shrink
        )
    ...
    def test_cosine_similarity_dense_adjusted(self):
        ...
        for row in range(data_matrix.shape[0]):
            ...
            data_matrix[row, :][nonzeroMask] -= np.mean(
                data_matrix[row, :][nonzeroMask]
            )
    ...
    def test_cosine_similarity_dense_pearson(self):
        ...
        cosine_similarity = Cosine_Similarity_Cython(
            data_matrix, topK=TopK, normalize=True, shrink=shrink, mode="pearson"
        )
        W_dense_Cython = cosine_similarity.compute_similarity()
    ...
    def test_cosine_similarity_dense_jaccard(self):
        ...
        W_dense_denominator = (
            W_dense_denominator + W_dense_denominator.T - W_dense_mul + shrink
        )
    ...
    def test_cosine_similarity_TopK(self):
        ...
        assert np.allclose(
            W_dense_Cython, W_dense_mul, atol=1e-4
        ), "W_sparse_Cython not matching control"
        assert np.allclose(
            W_dense_Python, W_dense_mul, atol=1e-4
        ), "W_dense_Python not matching control"
        assert np.allclose(
