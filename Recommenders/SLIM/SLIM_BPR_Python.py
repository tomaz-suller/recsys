...
class SLIM_BPR_Python(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(
        self, topK=100, epochs=25, lambda_i=0.0025, lambda_j=0.00025, learning_rate=0.05
    ):
        """
    ...
    def _run_epoch(self, n_epoch):
        ...
        for sample_num in range(self.n_users):
            ...
            self.item_item_S[pos_item_id, user_seen_items] += self.learning_rate * (
                sigmoid_gradient
                - self.lambda_i * self.item_item_S[pos_item_id, user_seen_items]
            )
            self.item_item_S[pos_item_id, pos_item_id] = 0

            self.item_item_S[neg_item_id, user_seen_items] -= self.learning_rate * (
                sigmoid_gradient
                - self.lambda_j * self.item_item_S[neg_item_id, user_seen_items]
            )
