        ...
        for currentItem in range(n_items):
            ...
            if len(nonzero_model_coef_value) > self.topK:
                ...
                relevant_items_partition = np.argpartition(
                    -np.abs(nonzero_model_coef_value), self.topK - 1, axis=0
                )[0 : self.topK]
                nonzero_model_coef_index = nonzero_model_coef_index[
                    relevant_items_partition
                ]
                nonzero_model_coef_value = nonzero_model_coef_value[
                    relevant_items_partition
                ]

            similarity_builder.add_data_lists(
                row_list_to_add=nonzero_model_coef_index,
                col_list_to_add=np.ones(len(nonzero_model_coef_index), dtype=np.int32)
                * currentItem,
...
def _partial_fit(
    ...
    for currentItem in items:
        ...
        if len(nonzero_model_coef_value) > topK:
            # Partition the data because this operation does not require to fully sort the data
            relevant_items_partition = np.argpartition(
                -np.abs(nonzero_model_coef_value), topK - 1, axis=0
            )[0:topK]
            nonzero_model_coef_index = nonzero_model_coef_index[
                relevant_items_partition
            ]
...
class MultiThreadSLIM_SLIMElasticNetRecommender(SLIMElasticNetRecommender):
    def fit(
        ...
        _pfit = partial(
            ...
            shm_shapes=[
                self.URM_train.indptr.shape,
                self.URM_train.indices.shape,
                self.URM_train.data.shape,
            ],
            shm_dtypes=[
                self.URM_train.indptr.dtype,
                self.URM_train.indices.dtype,
        ...
        with Pool(processes=self.workers) as pool:
            ...
            values, rows, cols = [], [], []
            for values_, rows_, cols_ in pool.imap_unordered(
                _pfit, itemchunks, pool_chunksize
            ):
        ...
        self.W_sparse = sps.csr_matrix(
            (values, (rows, cols)), shape=(self.n_items, self.n_items), dtype=np.float32
        )
