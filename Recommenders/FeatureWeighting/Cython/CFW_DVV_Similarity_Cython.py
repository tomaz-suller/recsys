...
class CFW_DVV_Similarity_Cython(
    ...
    def fit(
        ...
        **earlystopping_kwargs,
        ...
        from FeatureWeighting.Cython.CFW_DVV_Similarity_Cython_SGD import (
            CFW_DVV_Similarity_Cython_SGD,
        )

        if initialization_mode_D not in self.INIT_TYPE_VALUES:
            raise ValueError(
                "Value for 'initialization_mode_D' not recognized. Acceptable values are {}, provided was '{}'".format(
    ...
    def _generate_train_data(self):
        ...
        if zeros_to_add != 0.0:
            self.row_list = np.concatenate(
                (
                    np.array(self.S_matrix_target.row, dtype=np.int32),
                    np.zeros(zeros_to_add, dtype=np.int32),
                )
            )
            self.col_list = np.concatenate(
                (
                    np.array(self.S_matrix_target.col, dtype=np.int32),
                    np.zeros(zeros_to_add, dtype=np.int32),
                )
            )
    ...
    def _add_zeros_in_train_data_row_wise(self):
        ...
        while zeros_added_global < zeros_to_add_global:
            for current_item_row in range(self.n_items):
                ...
                while (
                    zeros_added_per_item < zeros_to_add_per_item
                    and zeros_added_global < zeros_to_add_global
                ):
                    ...
                    if new_coordinate not in nonzero_coordinates:
                        ...
                        self.row_list[numSamples + zeros_added_global] = (
                            current_item_row
                        )
        ...
        if self.verbose:
            print(
                "Added: {} zeros. Average per item is: {} ".format(
    ...
    def compute_W_sparse(self, use_D=True, use_V=True, model_to_use="best"):
        ...
        if use_V:
            ...
            for numItem in range(self.n_items):
                ...
                relevant_items_partition = (-V_weights).argpartition(self.topK - 1)[
                    0 : self.topK
                ]
                ...
                if self.verbose and (
                    ...
                    print(
                        "{}: Weighted similarity column {} ( {:2.0f} % ), {:.2f} column/sec, elapsed time {:.2f} min".format(
                            self.RECOMMENDER_NAME,
                            numItem,
                            numItem / self.n_items * 100,
                            columnPerSec,
