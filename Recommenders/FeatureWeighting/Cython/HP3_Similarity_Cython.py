...
class HP3_Similarity_Cython(
        ...
        **earlystopping_kwargs,
        ...
        if init_value <= 0:
            ...
            print(
                ...
                + ": Invalid init value, using default ("
                + str(self.INIT_VALUE)
                + ")"
            )

        # Import compiled module
        from FeatureWeighting.Cython.HP3_Similarity_Cython_SGD import (
            HP3_Similarity_Cython_SGD,
        )
        ...
        weights_initialization_D = (
            np.ones(self.n_features, dtype=np.float64) * init_value
        )

        # Instantiate fast Cython implementation
        self.HP3_Similarity = HP3_Similarity_Cython_SGD(
            self.row_list,
            self.col_list,
            self.data_list,
            self.n_features,
    ...
    def compute_W_sparse(self, model_to_use="best"):
        ...
        self.W_sparse = normalize(
            sps.csr_matrix((data, indices, indptr), shape=(self.n_items, self.n_items)),
            norm="l1",
            axis=1,
        )
