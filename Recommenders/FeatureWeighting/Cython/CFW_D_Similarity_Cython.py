...
class EvaluatorCFW_D_wrapper(object):
    def __init__(self, evaluator_object, ICM_target, model_to_use="best"):
        ...
        assert (
            model_to_use in ["best", "last"]
        ), "EvaluatorCFW_D_wrapper: model_to_use must be either 'best' or 'incremental'. Provided value is: '{}'".format(
            model_to_use
        )
...
class CFW_D_Similarity_Cython(
        ...
        if URM_train.shape[1] != ICM_train.shape[0]:
            raise ValueError(
                "Number of items not consistent. URM contains {} but ICM contains {}".format(
                    URM_train.shape[1], ICM_train.shape[0]
                )
            )
        ...
        if S_matrix_target.shape[0] != ICM_train.shape[0]:
            raise ValueError(
                "Number of items not consistent. S_matrix contains {} but ICM contains {}".format(
                    S_matrix_target.shape[0], ICM_train.shape[0]
                )
            )
    ...
    def fit(
        ...
        **earlystopping_kwargs,
        ...
        from Recommenders.FeatureWeighting.Cython.CFW_D_Similarity_Cython_SGD import (
            CFW_D_Similarity_Cython_SGD,
        )
    ...
    def _generate_train_data(self):
        ...
        self._print(
            "Content S structure has {} out of {} ({:4.1f}%) nonzero collaborative cells".format(
