...
class BaseItemCBFRecommender(_BaseRecommender):
    ...
    def __init__(self, URM_train, ICM_train, verbose=True):
        ...
        assert (
            self.n_items == ICM_train.shape[0]
        ), "{}: URM_train has {} items but ICM_train has {}".format(
        ...
        if self._cold_item_CBF_mask.any():
            print(
                "{}: ICM Detected {} ({:4.1f}%) items with no features.".format(
                    self.RECOMMENDER_NAME,
                    self._cold_item_CBF_mask.sum(),
...
class BaseUserCBFRecommender(_BaseRecommender):
    ...
    def __init__(self, URM_train, UCM_train, verbose=True):
        ...
        assert (
            self.n_users == UCM_train.shape[0]
        ), "{}: URM_train has {} users but UCM_train has {}".format(
        ...
        if self._cold_user_CBF_mask.any():
            print(
                "{}: UCM Detected {} ({:4.1f}%) cold users.".format(
                    self.RECOMMENDER_NAME,
                    self._cold_user_CBF_mask.sum(),
