...
class _BaseLightFMWrapper(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def _init_model(
        self, loss, sgd_mode, n_components, item_alpha, user_alpha, learning_rate
    ):
    ...
    def fit(
        ...
        **earlystopping_kwargs,
        ...
        if loss not in self.LOSS_VALUES:
            raise ValueError(
                "Value for 'loss' not recognized. Acceptable values are {}, provided was '{}'".format(
        ...
        if sgd_mode not in self.SGD_MODE_VALUES:
            raise ValueError(
                "Value for 'sgd_mode' not recognized. Acceptable values are {}, provided was '{}'".format(
                    self.SGD_MODE_VALUES, sgd_mode
                )
            )

        self._init_model(
            loss, sgd_mode, n_components, item_alpha, user_alpha, learning_rate
        )
