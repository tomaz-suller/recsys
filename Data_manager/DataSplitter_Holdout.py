...
class DataSplitter_Holdout(_DataSplitter):
    ...
    def __init__(
        ...
        if split_interaction_quota_list is None:
            ...
            self._print(
                "input_split_item_quota_list not provided, using default '{}'".format(
        ...
        assert all(
            split_quota >= 0.0 and split_quota <= 100
            for split_quota in split_interaction_quota_list
        ), "{}: input_split_item_quota_list must contain values between 0 and 100".format(
            self.DATA_SPLITTER_NAME
        )
        assert (
    ...
    def _get_split_subfolder_name(self):
        ...
        return "holdout_{}_{}/".format(
            "_".join(
                str(split_quota)
                for split_quota in self.input_split_interaction_quota_list
            ),
            user_wise_string,
        )
        ...
        if len(self.dataReader_object.get_loaded_UCM_names()) > 0:
            for UCM_name, UCM_object in self.SPLIT_UCM_DICT.items():
                ...
                statistics_string = "\tUCM name: {}, Num features: {}, feature occurrences: {}, density {:.2E}".format(
                    UCM_name, n_features, UCM_object.nnz, compute_density(UCM_object)
                )
    ...
    def _split_data_from_original_dataset(self, save_folder_path):
        ...
        train_quota, validation_quota, test_quota = (
            ...
            if user_to_remove.sum() > 0:
                self._print(
                    "Removing {} ({:.2f} %) of {} users because they have no interactions in train data.".format(
                        user_to_remove.sum(),
                        user_to_remove.sum() / n_users * 100,
                        n_users,
                    )
                )
                ...
                self.SPLIT_GLOBAL_MAPPER_DICT["user_original_ID_to_index"] = (
                    reconcile_mapper_with_removed_tokens(
                        self.SPLIT_GLOBAL_MAPPER_DICT["user_original_ID_to_index"],
                        np.arange(0, len(user_to_remove), dtype=np.int32)[
                            user_to_remove
                        ],
                    )
                )
    ...
    def _compute_real_split_interaction_quota(self):
        ...
        self.actual_split_interaction_quota_list = [None] * len(
            self.input_split_interaction_quota_list
        )
    ...
    def _verify_data_consistency(self):
        ...
        assert all(
            URM_name in self.SPLIT_URM_DICT for URM_name in self._SPLIT_URM_NAME_LIST
        ...
        assert (
            self.SPLIT_URM_DICT["URM_validation"].nnz == 0
            and self.input_split_interaction_quota_list[1] == 0.0
        ) or (
            self.SPLIT_URM_DICT["URM_validation"].nnz != 0
            and self.input_split_interaction_quota_list[1] > 0.0
