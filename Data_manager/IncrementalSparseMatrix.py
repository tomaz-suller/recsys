...
class IncrementalSparseMatrix_FilterIDs(IncrementalSparseMatrix):
    ...
    def __init__(
        ...
        if on_new_col == "add":
            assert (
                preinitialized_col_mapper is None
                or isinstance(preinitialized_col_mapper, dict)
            ), "IncrementalSparseMatrix: if on_new_col is 'add' then preinitialized_col_mapper must be either 'None' or contain a dictionary"

        if on_new_row == "add":
            assert (
                preinitialized_row_mapper is None
                or isinstance(preinitialized_row_mapper, dict)
