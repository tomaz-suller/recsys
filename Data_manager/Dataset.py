...
class Dataset(object):
    ...
    def verify_data_consistency(self):
        ...
        for URM_name, URM_object in self.AVAILABLE_URM.items():
            assert URM_object.nnz != 0, (
                print_preamble + "Number of interactions in {} is 0".format(URM_name)
            )
            assert not np.isnan(URM_object.data).any(), (
                print_preamble + "Contains NANs in {}".format(URM_name)
            )
    ...
    def _remove_items_and_users(self, items_to_remove=None, users_to_remove=None):
        ...
        if items_to_remove is not None:
            ...
            for URM_name, URM_obj in self.AVAILABLE_URM.items():
                self.AVAILABLE_URM[URM_name] = URM_obj.tocsc()[
                    :, items_to_keep_mask
                ].tocsr()
