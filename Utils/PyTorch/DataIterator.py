...
class InteractionIterator(object):
    ...
    def __init__(
        self, URM_train, positive_quota, batch_size=1, set_n_samples_to_draw=None
    ):
        super(InteractionIterator, self).__init__()
        ...
        self.n_samples_to_draw = (
            self.n_samples_available
            if set_n_samples_to_draw is None
            else set_n_samples_to_draw
        )
    ...
    def __next__(self):
        ...
        this_batch_size = min(
            self.batch_size, self.n_samples_to_draw - self.n_sampled_points
        )
        self.n_sampled_points += this_batch_size
        ...
        negative_interaction_flag = (
            np.random.rand(self.batch_size) > self.positive_quota
        )
        ...
        for i_batch in negative_interaction_flag.nonzero()[0]:
            ...
            while not negative_item_selected:
                ...
                while (
                    index < n_seen_items
                    and self.URM_train.indices[start_pos_seen_items + index]
                    < negative_item
                ):
                    index += 1
                ...
                if (
                    index == n_seen_items
                    or self.URM_train.indices[start_pos_seen_items + index]
                    > negative_item
                ):
...
class BPRIterator(object):
    ...
    def __init__(
        ...
        self.warm_user_index_to_original_id = np.arange(0, self.n_users)[
            np.ediff1d(self.URM_train.indptr) > 0
        ]
        ...
        if self.n_negatives_per_positive == 1:
            ...
            self.batch_negative_item = torch.empty(
                (self.batch_size, self.n_negatives_per_positive), dtype=torch.long
            )
    ...
    def __next__(self):
        ...
        for i_batch in range(
            0, min(self.batch_size, self.n_samples_to_draw - self.n_samples_drawn)
        ):
            ...
            for negative_item_batch_index in range(self.n_negatives_per_positive):
                ...
                if self.n_negatives_per_positive == 1:
                    ...
                    self.batch_negative_item[i_batch, negative_item_batch_index] = (
                        negative_item
                    )
...
class InteractionAndNegativeIterator(object):
    ...
    def __next__(self):
        ...
        this_batch_size = min(
            self.batch_size, self.n_samples_to_draw - self.n_samples_drawn
        )
